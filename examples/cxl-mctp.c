...
static int test_fmapi_get_host_dc_reg_config(struct cxlmi_endpoint *ep)
{
	...
	for (i = 0; i < dc_reg_config_rsp->num_regs_returned; i++) {
		printf("\t\tRegion %d:\n", i);
		printf("\t\t\tBase: %lu\n", dc_reg_config_rsp->region_configs->base);
		printf("\t\t\tBlk_sz: %lu\n", dc_reg_config_rsp->region_configs->block_size);
		printf("\t\t\tLen: %lu\n", dc_reg_config_rsp->region_configs->region_len);
	}
						...
						uint32_t start_ext_ind)
{
	...
	req.start_ex_indx = start_ext_ind;
...
static int test_fmapi_initiate_dc_release(struct cxlmi_endpoint *ep)
{
	...
	rls_req->host_id = 0;
	rls_req->flags = CXL_EXTENT_REMOVAL_POLICY_PRESCRIPTIVE;
	rls_req->length = 2097152;
	rls_req->ext_count = 1;
	rls_req->extents[0].start_dpa = 268435456;
	rls_req->extents[0].len = 2097152;
