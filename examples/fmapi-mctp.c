...
static int parse_extents(extent *ext_list, bool add)
{
    ...
    printf("How many extents to %s? (max. %d) ", add ? "add" : "release", MAX_EXTENTS);
...
/*
 * Creates and sends 0x5604 FM Initiate DC Add to the given endpoint using extents
 * specified in ext_list.
 *
...
/*
 * Creates and sends 0x5605 FM Initiate DC Release to the given endpoint using extents
 * specified in ext_list.
 *
 * @param: num_extents - number of extents in ext_list
 * @param: ext_list - list of extents to include in add request
...
/*
 * Splits input cmd into X tokens, using ' ' as the delimiter. Allocates space for
 * each token. Allocated space must be freed by the caller. Returns number
...
/*
 * Parent proc forks and waits on child proc to exec the given file
 *
 * @param: file - file to execute
 * @param: argv - arguments for file
...
/*
 * Create and online a DAX Device using the commands defined in DAX_DEVICE_CMDS
 *
 * @param: buf - user input buf
 * @param: rc - pointer through which to return error codes
 */
static int create_dax_device(struct cxlmi_ctx *ctx) {
    ...
    if (input_is_yes(buf, &rc)) {
        for (i = 0; i < NUM_CMDS; i++) {
            ...
            if (i < NUM_CMDS - 1) {
                printf("Next cmd: '%s' ------------- continue? [y/n] ", DAX_DEVICE_CMDS[i + 1]);
                if (!input_is_yes(buf, &rc)) {
                    break;
                }
            }
