...
/* CXL r3.1 7.6.7.6.2 Get Host DC Region Configuration (Opcode 5601h) */
...
struct cxlmi_cmd_fmapi_get_host_dc_reg_config_rsp {
	...
	struct {
		...
		uint64_t region_len;
		uint64_t block_size;
		uint8_t dsmadhandle;	// TODO: fill out according to spec Table 7-71. DC Region Configuration
		uint8_t rsvd[3];
...
/* CXL r3.1 Section 7.6.7.6.3 Set DC Region Configuration (Opcode 5602h) */
struct cxlmi_cmd_fmapi_set_dc_reg_config {
	uint8_t reg_id;
	uint8_t rsvd[3];
	uint64_t block_sz;
