				    ...
				    struct cxlmi_cmd_fmapi_get_host_dc_reg_config_rsp *ret)
{
	int i, rc = -1;
	void *p;
	struct cxlmi_cmd_fmapi_get_host_dc_reg_config_req *req_pl;
	struct cxlmi_cmd_fmapi_get_host_dc_reg_config_rsp *rsp_pl;
	ssize_t req_sz, rsp_sz, rsp_sz_min;
	...
	req_pl = (struct cxlmi_cmd_fmapi_get_host_dc_reg_config_req *)req->payload;
	req_pl->host_id = in->host_id;
	req_pl->region_cnt = in->region_cnt;
	req_pl->starting_reg_index = in->starting_reg_index;
	...
	rsp_pl = (struct cxlmi_cmd_fmapi_get_host_dc_reg_config_rsp *)rsp->payload;
	memset(ret, 0, sizeof(*ret));

	ret->host_id = rsp_pl->host_id;
	ret->num_available_regs = rsp_pl->num_available_regs;
	ret->num_regs_returned = rsp_pl->num_regs_returned;
	for (i = 0; i < ret->num_regs_returned; i++) {
		ret->region_configs[i].base = le64_to_cpu(rsp_pl->region_configs[i].base);
		ret->region_configs[i].decode_len =
			le64_to_cpu(rsp_pl->region_configs[i].decode_len) * CXL_CAPACITY_MULTIPLIER;
		ret->region_configs[i].region_len = le64_to_cpu(rsp_pl->region_configs[i].region_len);
		ret->region_configs[i].block_size = le64_to_cpu(rsp_pl->region_configs[i].block_size);
		ret->region_configs[i].dsmadhandle = rsp_pl->region_configs[i].dsmadhandle;
		ret->region_configs[i].sanitize_on_rls = rsp_pl->region_configs[i].sanitize_on_rls;
	}
	p = (void *)&rsp_pl->region_configs[i];
	ret->num_extents_supported = le32_to_cpu(*(uint32_t *)p);
	p += sizeof(uint32_t);
...
CXLMI_EXPORT int cxlmi_cmd_fmapi_set_dc_reg_config(struct cxlmi_endpoint *ep,
				    struct cxlmi_tunnel_info *ti,
				    struct cxlmi_cmd_fmapi_set_dc_reg_config *in)
{
	struct cxlmi_cmd_fmapi_set_dc_reg_config *req_pl;
	_cleanup_free_ struct cxlmi_cci_msg *req = NULL;
	struct cxlmi_cci_msg rsp;
	size_t req_sz;
	int rc = 0;
	...
	req_pl = (struct cxlmi_cmd_fmapi_set_dc_reg_config *)req->payload;
	req_pl->reg_id = in->reg_id;
	req_pl->block_sz = in->block_sz;
...
CXLMI_EXPORT int cxlmi_cmd_fmapi_get_dc_reg_ext_list(struct cxlmi_endpoint *ep,
				    struct cxlmi_tunnel_info *ti,
				    struct cxlmi_cmd_fmapi_get_dc_reg_ext_list_req *in,
				    struct cxlmi_cmd_fmapi_get_dc_reg_ext_list_rsp *ret)
{
	int i, rc;
	size_t req_sz, rsp_sz, min_rsp_sz;
	struct cxlmi_cmd_fmapi_get_dc_reg_ext_list_req *req_pl;
	struct cxlmi_cmd_fmapi_get_dc_reg_ext_list_rsp *rsp_pl;
	_cleanup_free_ struct cxlmi_cci_msg *req = NULL;
	_cleanup_free_ struct cxlmi_cci_msg *rsp = NULL;
	...
	req_pl = (struct cxlmi_cmd_fmapi_get_dc_reg_ext_list_req *)req->payload;
	req_pl->host_id = in->host_id;
	req_pl->start_ex_indx = in->start_ex_indx;
	req_pl->extent_count = in->extent_count;
	...
	rsp_pl = (struct cxlmi_cmd_fmapi_get_dc_reg_ext_list_rsp *) rsp->payload;
	ret->host_id = le16_to_cpu(rsp_pl->host_id);
	ret->start_ex_indx = le32_to_cpu(rsp_pl->start_ex_indx);
				...
				struct cxlmi_cmd_fmapi_dc_list_tags_rsp *ret)
{
	...
	req_pl->start_ind = in->start_ind;
	req_pl->max_tags = in->max_tags;

	num_tag_infos = req_pl->max_tags;
	rsp_sz = sizeof(*rsp) + sizeof(*rsp_pl) + num_tag_infos * sizeof(rsp_pl->tags_list[0]);
	rsp = calloc(1, rsp_sz);
	if (!rsp)
		return -1;
	rc = send_cmd_cci(ep, ti, req, req_sz, rsp, rsp_sz, rsp_sz);
	...
	for (i = 0; i < ret->num_tags_returned; i++) {
		...
		memcpy(&ret->tags_list[i].pending_ref_bitmap, &rsp_pl->tags_list[i].pending_ref_bitmap, 0x20);
	}
