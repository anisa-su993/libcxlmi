...
def add_args(parser):
    ...
    parser.add_argument('-c', '--check-rsp', action='store_true', required=False, help='check expected response payloads?')
    parser.add_argument('-r', '--rc', type=int, required=False, default=0, help='expected return code')
    parser.add_argument('-d', '--mctpd', type=str, required=False, default='codeconstruct', help='mctpd compile flag (openbmc | codeconstruct)')
...
if __name__ == "__main__":
    ...
    context.rc = args.return_code
    context.check_rsp = args.check_rsp
